        ...
        if exc_raised:
            ...
            for res in response:
                if res[0]:
                    ...
                    ret_msgs.append(f"User {self.users[nu].username} subscribed to {settings.MAILINGLIST_NAME}")
                    ...
                    if res[1].status == 409:
                        ...
                        await self.session.close()
                        ret_msgs.append(f"Error subscribing user {self.users[nu].username} to {settings.MAILINGLIST_NAME}: {repr(res[1])}")
                nu += 1
