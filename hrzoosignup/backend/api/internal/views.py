from .view_croris import CroRISInfo
from .view_sshkeys import SshKeys
from .view_projects import ProjectsResearch, ProjectsGeneral, Projects, ProjectsRole, CanSubmitInstituteProject
from .view_project_extend import ProjectExtend
from .view_invite import Invites, InvitesLink, InvitesSent
from .view_software import ScienceSoftware
from .view_userproject import UsersProjects, UsersProjectsInternal
from .view_users import UsersInfo, UsersInfoInactive, UsersInfoOps, UserInfo
from .view_accounting import ResourceUsage, ProjectUsagePerUser, ProjectUsage
