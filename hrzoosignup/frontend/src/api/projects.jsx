...
export async function fetchNrProjectsLead()
{
  let error_msg = ''

  try {
    let response = await fetch(`${url_api_prefix}/api/v1/internal/projects/role/lead`)
  ...
  if (error_msg)
    throw new Error(`Error fetching NRProjects Lead data: ${error_msg}`)
}
...
export async function canSubmitInstituteProject()
{
  let error_msg = ''

  try {
    let response = await fetch(`${url_api_prefix}/api/v1/internal/can-submit-institute-project`)
...
export async function extendProject(projectId, data, csrftoken)
{
  ...
  try {
    let response = await fetch(`${url_api_prefix}/api/v1/internal/project-extend/${projectId}`, {
      method: 'POST',
      mode: 'cors',
      cache: 'no-cache',
      credentials: 'same-origin',
